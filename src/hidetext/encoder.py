...
class EncodeResult:
    ...
    def bits_per_token(self) -> float:
        if not self.tokens:
            return 0.0
        return (len(self.packet) * 8) / len(self.tokens)
...
class StegoEncoder:
    ...
    def encode(self, plaintext: str, *, passphrase: str, prompt: str) -> EncodeResult:
        ...
        return EncodeResult(
            text=self.backend.render(tokens),
            tokens=tuple(tokens),
            packet=packet,
            config_fingerprint=config_fingerprint,
        ...
        *,
        ...
        while not encoder.finished:
            ...
            if quantized.allows_encoding:
                ...
                chosen_token = quantized.entries[index].token
                encoding_steps += 1
                embedded_bits += gained_bits
            else:
                chosen_token = quantized.top.token
